...
function Invoke-SafeAdblock {
    ...
    if (-not $isAdmin) {
        Write-Log "[ERROR] Administrator-Rechte erforderlich für Hosts-Datei Änderungen" -Color Red
    ...
    try {
        ...
        $choice = if ($Mode -gt 0) { "$Mode" } else { Read-Host "`nWahl [1-4/x]" }
        ...
        switch ($choice.ToLower()) {
            ...
            '3' {
                ...
                if (Test-Path $backupPath) {
                    $confirm = if ($Force) { "j" } else { Read-Host "`nHost-Datei auf Original zurücksetzen? [j/n]" }
            ...
            '4' {
                ...
                Write-Log "[*] Öffne Host-Datei in Notepad..." -Color Blue
                try {
                    ...
                    Write-Log "[OK] Host-Datei geoeffnet - Manuelle Bearbeitung möglich" -Color Green
                } catch {
