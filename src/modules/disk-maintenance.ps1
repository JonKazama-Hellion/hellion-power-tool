...
function Invoke-CheckDisk {
    ...
    foreach ($drive in $drives) {
        ...
        Write-Host "$dispLetter " -ForegroundColor Cyan -NoNewline
    ...
    $driveChoice = if ($DriveLetter -ne '') {
        # GUI-Modus: Finde den Index des übergebenen Laufwerksbuchstabens
        $matchIndex = 0
        $found = $false
        foreach ($d in $drives) {
            $matchIndex++
            if ($d.DeviceID -eq "$DriveLetter" -or $d.DeviceID -eq "${DriveLetter}:" -or $d.DeviceID.TrimEnd(':') -eq $DriveLetter.TrimEnd(':')) {
                $found = $true
                break
            }
        }
        if ($found) { "$matchIndex" } else { "x" }
    } else { Read-Host "`nLaufwerk wählen [1-$($drives.Count)] oder [x] zum Abbrechen" }
    ...
    try {
        ...
        $modeChoice = if ($Mode -gt 0) { "$Mode" } else { Read-Host "`nModus wählen [1-3]" }
        ...
        switch ($modeChoice) {
            ...
            '2' {
                ...
                $confirm = if ($Force) { "j" } else { Read-Host "Fortfahren? [j/n]" }
            ...
            '3' {
                ...
                Write-Host "WARNUNG: " -ForegroundColor Red -NoNewline
                Write-Host "Reparatur-Modus kann Datenverlust verursachen!" -ForegroundColor White
                $confirm = if ($Force) { "j" } else { Read-Host "Wirklich fortfahren? [j/n]" }
                if ($confirm -ne 'j' -and $confirm -ne 'J') {
...
function Invoke-SystemFileChecker {
    ...
    try {
        ...
        # Progress-Monitor während SFC laeuft
        $progressTimer = 0
        $dots = ""
        $jobStarted = $false
...
function Invoke-DISMRepair {
    ...
    $choice = if ($Mode -gt 0) { "$Mode" } else { Read-Host "`nWahl [1-3/x]" }
