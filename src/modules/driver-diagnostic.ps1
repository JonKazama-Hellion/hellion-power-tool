...
function Enable-DriverVerifier {
    ...
    $confirm = if ($Force) { "j" } else { Read-Host "Möchtest du Driver Verifier Informationen anzeigen? (j/n)" }
...
function Analyze-ENEDriverProblem {
    ...
    if ($eneSystemDriver) {
        ...
        $eneSystemDriver | ForEach-Object {
            ...
            # Variable initialisieren für Scope-Sicherheit
            $deviceErrors = $null
    ...
    if ($eneSystemDriver -and ($eneSystemDriver.State -eq "Running")) {
        ...
        Write-Host ""
        $autoFix = if ($Force) { "j" } else { Read-Host "Soll Windows die ENE-Treiber automatisch reparieren? (j/n)" }
...
function Remove-ENEDriverForce {
    ...
    $confirm = if ($Force) { "CONFIRM" } else { Read-Host "Trotzdem fortfahren? (CONFIRM)" }
    ...
    Write-Host ""
    $continueAnyway = if ($Force) { "j" } else { Read-Host "Ohne Backup fortfahren? (j/n)" }
