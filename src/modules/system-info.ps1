    ...
    try {
        ...
        try {
            ...
            if ($antivirusStatus.RealTimeProtectionEnabled) {
                Write-Log "[INFO] Echtzeit-Antivirus aktiv - Langsamere Operationen möglich" -Color Yellow
            }
        } catch {
...
"@
        ...
        Write-Log "[INFO] Bericht-Größe: $([math]::Round((Get-Item $reportPath).Length / 1KB, 2)) KB" -Color Gray
