...
function Invoke-ComprehensiveCleanup {
    ...
    # Cleanup-Modus abfragen (Mode-Parameter überspringt Menue)
    if ($Mode -gt 0) {
        $cleanupMode = "$Mode"
    } elseif (-not $script:AutoApproveCleanup) {
        Write-Host ""
    ...
    foreach ($service in $servicesToDisable) {
        try {
            ...
            if ($svc -and $svc.Status -eq 'Running') {
                ...
                if ($choice -ne 'j') {
                    Write-Log "  [SKIP] $($service.Description) übersprungen" -Color Gray
                    continue
                }
...
function Get-UnusedPrograms {
    ...
    Write-Log "Analysiert Programme basierend auf Nutzung, Alter und Größe" -Color Yellow
    ...
    try {
        ...
        foreach ($program in $programs) {
            ...
            # 2. Analyse: Größe (nur als Hinweis, nicht automatisch ungenutzt)
            $sizeInMB = 0
            if ($program.EstimatedSize -and $program.EstimatedSize -gt 0) {
                $sizeInMB = [math]::Round($program.EstimatedSize / 1024, 1)
                
                # Größe allein macht Programme NICHT ungenutzt
                ...
                # Nur Größe-Warnung bei sehr grossen Programmen (ohne als ungenutzt zu markieren)
        ...
        # Ergebnisse sortieren nach Risiko und Größe
        ...
        if ($unusedPrograms.Count -gt 0) {
            ...
            Write-Log "Gefunden: $($unusedPrograms.Count) Programme die möglicherweise entfernt werden können" -Color White
            ...
            for ($i = 0; $i -lt $displayCount; $i++) {
                ...
                Write-Information "[INFO]       Größe: $($prog.Size)" -InformationAction Continue
            ...
            if ($unusedPrograms.Count -gt $displayCount) {
                ...
                for ($i = $displayCount; $i -lt $unusedPrograms.Count; $i++) {
                    ...
                    Write-Log "     Version: $($prog.Version) | Größe: $($prog.Size) | Risiko: $($prog.RiskLevel)" -Level "DEBUG"
