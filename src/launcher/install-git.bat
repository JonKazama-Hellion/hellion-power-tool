...
title Git Installation v8.0.0.0
color 0E

echo.
echo   ================================================================
echo          GIT INSTALLATION v8.0.0.0
...
if %errorlevel%==0 (
    set "GIT_FOUND=1"
    set "GIT_PATH_OK=1"
    echo   [OK] Git via PATH verfügbar
...
if "!GIT_FOUND!"=="1" (
    if "!GIT_PATH_OK!"=="1" (
        echo   Status: Git ist vollständig funktionsfaehig
        if not "!GIT_VERSION!"=="" (
            echo   !GIT_VERSION!
        )
    ) else (
    ...
    echo.
    choice /c JNP /n /m "   Option wählen [J/N/P]: "
    if errorlevel 3 (
        echo.
        echo   PATH-Reparatur...
        if "!GIT_LOCATION!"=="" (
            echo   [ERROR] Git-Pfad konnte nicht bestimmt werden
            echo   Installiere Git neu für automatische PATH-Konfiguration
...
if %INSTALL_RESULT%==0 (
    ...
    if "!INSTALL_SUCCESS!"=="1" (
        ...
        echo   2. Öffne neue Konsole (wichtig für PATH-Update)
        ...
        echo   Git ist möglicherweise installiert aber noch nicht verfügbar
...
:END
echo.
echo   Git wird für GitHub-Operations im Hellion Tool benötigt
