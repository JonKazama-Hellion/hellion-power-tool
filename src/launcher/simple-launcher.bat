...
title Hellion Simple Launcher v8.0.0.0
color 0A
...
echo.
echo   ================================================================
echo          HELLION SIMPLE LAUNCHER v8.0.0.0
...
REM Intelligente Root-Verzeichnis Bestimmung (funktioniert von ROOT und /src/launcher)

REM Prüfe ob wir bereits im Root-Verzeichnis sind
if exist "%CD%\src\hellion_main.ps1" if exist "%CD%\config\version.txt" (
    set "ROOT_DIR=%CD%"
...
if "!ROOT_DIR!"=="" (
    ...
    echo     2. Aus dem src\launcher\-Unterverzeichnis
    echo.
    ...
    if exist "C:\Program Files\PowerShell\7\pwsh.exe" (
        ...
        if exist "%LOCALAPPDATA%\Microsoft\WindowsApps\pwsh.exe" (
            ...
            if errorlevel 2 (
                ...
                echo.
                echo   Prüfe Verfügbarkeit nach Installation...
                timeout /t 3 /nobreak >nul

                REM Multi-Level PS7 Detection (robust)
                where pwsh >nul 2>&1
                if !errorlevel!==0 (
                    echo   [OK] PowerShell 7 verfügbar (PATH)
                    set "USE_PS=pwsh"
                    set "PS_VERSION=7"
                    goto :PS_DETECTION_DONE
                ) else (
                    if exist "C:\Program Files\PowerShell\7\pwsh.exe" (
                        echo   [OK] PowerShell 7 verfügbar (Programm-Pfad)
                        set "USE_PS=C:\Program Files\PowerShell\7\pwsh.exe"
                        set "PS_VERSION=7"
                        goto :PS_DETECTION_DONE
                    ) else (
                        if exist "!LOCALAPPDATA!\Microsoft\WindowsApps\pwsh.exe" (
                            echo   [OK] PowerShell 7 verfügbar (Store)
                            set "USE_PS=!LOCALAPPDATA!\Microsoft\WindowsApps\pwsh.exe"
                            set "PS_VERSION=7"
                            goto :PS_DETECTION_DONE
                        ) else (
...
echo   ----------------------------------------------------------------
echo.
choice /c 123450 /n /m "   Option wählen [1/2/3/4/5/0]: "
...
echo   Prüfe PowerShell nach Installation...
where pwsh >nul 2>&1
if %errorlevel%==0 (
    echo   [OK] PowerShell 7 jetzt verfügbar
    set "USE_PS=pwsh"
    set "PS_VERSION=7"
) else (
    if exist "C:\Program Files\PowerShell\7\pwsh.exe" (
...
if not exist "src\hellion_main.ps1" (
    echo   [ERROR] src\hellion_main.ps1 nicht gefunden!
...
if "%DEBUG_MODE%"=="1" (
    echo   Modus: Debug
    "%USE_PS%" -NoProfile -ExecutionPolicy Bypass -File src\hellion_main.ps1 -DebugMode
) else (
    echo   Modus: Normal
    "%USE_PS%" -NoProfile -ExecutionPolicy Bypass -File src\hellion_main.ps1 -ForceDebugLevel 0
)
...
REM Methode 1: Standard-Annahme - Launcher ist in src/launcher/-Subdir
...
if not exist "!TEST_DIR!\src\launcher" (
    set "%RESULT_VAR%=NO"
    goto :EOF
)
if not exist "!TEST_DIR!\src\modules" (
    set "%RESULT_VAR%=NO"
    goto :EOF
)
