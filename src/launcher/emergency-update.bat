...
echo.
echo   ================================================================
echo          HELLION EMERGENCY UPDATER v8.0.0.0
echo          Hellion Online Media
echo   ================================================================
echo.
echo   Dieser Notfall-Updater behebt Auto-Update Probleme
echo   für Benutzer von v7.1.0 bis v7.1.x
echo.
echo   WICHTIG: NICHT als Administrator ausführen!
...
if "!ROOT_DIR!"=="" (
    ...
    echo.
    echo   Empfehlung:
    echo   1. Kopiere emergency-update.bat in den src/launcher/-Ordner
...
set "MAIN_SCRIPT=!ROOT_DIR!\src\hellion_main.ps1"
...
if %CLONE_RESULT% neq 0 (
    ...
    echo   https://github.com/JonKazama-Hellion/hellion-power-tool/blob/main/src/launcher/update-check.bat
    cd /d "%~dp0"
...
cd hellion-fix
git checkout HEAD -- src/launcher/update-check.bat >nul 2>&1

if exist "src\launcher\update-check.bat" (
    echo   [OK] Reparierte Datei geladen!

    REM Kopiere die reparierte Datei zurück
    copy "src\launcher\update-check.bat" "%~dp0update-check.bat" >nul
    if !errorlevel!==0 (
        echo   [OK] update-check.bat erfolgreich repariert!
        echo.
        echo   Vollautomatisches Update-System ist jetzt verfügbar:
...
call "src\launcher\update-check.bat"
...
echo   Du kannst diesen emergency-update.bat jetzt löschen.
echo.
pause
goto :EOF
