...
# Pfade definieren
$toolPath = Join-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) "START.bat"
$desktopPath = [Environment]::GetFolderPath("Desktop")
$shortcutPath = Join-Path $desktopPath "Hellion Power Tool.lnk"

# Custom Icon Pfade
$rootPath = Split-Path (Split-Path $PSScriptRoot -Parent) -Parent
$customIco = Join-Path $rootPath "assets\icons\Gmark.ico"
$customPng = Join-Path $rootPath "assets\icons\Gmark.png"
...
try {
    ...
    $Shortcut.Description = "Hellion Power Tool v8.0.0.0 - System-Optimierung und Reparatur"
    ...
    if (Test-Path $shortcutPath) {
        Write-Host ""
        Write-Host "[SUCCESS] Desktop-Verknüpfung erfolgreich erstellt!" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "[ERROR] Verknüpfung konnte nicht erstellt werden" -ForegroundColor Red
    Write-Host "[GRUND] $($_.Exception.Message)" -ForegroundColor Yellow
    Write-Host "[LOESUNG] Versuche manuell oder prüfe Berechtigungen" -ForegroundColor Cyan
}
