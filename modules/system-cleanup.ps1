    ...
    try {
        ...
        foreach ($program in $programs) {
            ...
            if ($program.EstimatedSize -and $program.EstimatedSize -gt 0) {
                ...
                if ($sizeInMB -gt 5000 -and $isUnused) {  # >5GB UND bereits als ungenutzt markiert
                    $reasons += "Sehr gross ($sizeInMB MB)"
                    $riskLevel = "High"
                } elseif ($sizeInMB -gt 2000 -and $isUnused) {  # >2GB UND bereits als ungenutzt markiert
                ...
                if ($sizeInMB -gt 10000 -and -not $isUnused) {  # >10GB
            ...
            if ($isUnused -and $reasons.Count -gt 0) {
        ...
        if ($unusedPrograms.Count -gt 0) {
            ...
            for ($i = 0; $i -lt $displayCount; $i++) {
                ...
                Write-Host "`n  [$($i+1)] $($prog.Name)" -ForegroundColor $riskColor
                if ($prog.Publisher) { Write-Host "      Publisher: $($prog.Publisher)" -ForegroundColor Gray }
                if ($prog.Version) { Write-Host "      Version: $($prog.Version)" -ForegroundColor Gray }
                Write-Host "      Groesse: $($prog.Size)" -ForegroundColor Gray
                Write-Host "      Grund: $($prog.Reasons)" -ForegroundColor DarkGray
                Write-Host "      Risiko: $($prog.RiskLevel)" -ForegroundColor $riskColor
            }
            
            if ($unusedPrograms.Count -gt $displayCount) {
                $remainingCount = $unusedPrograms.Count - $displayCount
                Write-Host "`n  [INFO] ... und $remainingCount weitere Programme (siehe Log-Datei)" -ForegroundColor Gray
            ...
            Write-Host "`n[STATISTIKEN]" -ForegroundColor Cyan
            Write-Host "  Geschaetzte Speicher-Einsparung: $([math]::Round($totalSize, 1)) MB ($([math]::Round($totalSize/1024, 2)) GB)" -ForegroundColor Yellow
            Write-Host "  Hohes Risiko (empfohlen zu entfernen): $highRiskCount Programme" -ForegroundColor Red
            Write-Host "  Mittleres Risiko (pruefen): $mediumRiskCount Programme" -ForegroundColor Yellow
            
            Write-Host "`n[EMPFEHLUNG] Programme manuell pruefen - Automatische Deinstallation nicht implementiert" -ForegroundColor Green
