...
if (-not $isAdmin) {
    ...
    Write-Information "[INFO] Einige Funktionen werden möglicherweise nicht funktionieren." -InformationAction Continue
    ...
    Write-Information "[OK] Administrator-Rechte verfügbar" -InformationAction Continue
}

# Script Path Detection
$scriptRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
$rootPath = Split-Path -Parent $scriptRoot
$mainScript = Join-Path $rootPath "src\hellion_main.ps1"
